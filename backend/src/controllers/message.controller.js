...
const markMessageAsSeen = asyncHandler(async (req, res) => {
  ...
  if (senderSocketId) {
    getIO().to(senderSocketId).emit("updatedMessages", updatedMessages);
  }

  return res.status(200).json(new ApiResponse(200, updatedMessages));
});
...
const deleteForMe = asyncHandler(async (req, res) => {
  ...
  const alreadyDeleted = message.deletedFor.some((id) => id.equals(userId));
...
const deleteForEveryone = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, deleteMsg, "message deleted for everyone"));
});
